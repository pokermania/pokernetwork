...
class PokerExplain:
    ...
    def __init__(self, *args, **kwargs):
        self.log = PokerExplain.log.get_instance(self, refs=[
            ('Explain', self, lambda explain: explain.serial if explain.serial else None)
        ])
        self.serial = 0
    ...
    def explain(self, packet):
        ...
        if game:
            if packet.type == PACKET_POKER_START:
                ...
                player = game.addPlayer(packet.serial, seat=packet.seat, name=packet.name)
                if player:
            ...
            if not (packet.type == PACKET_POKER_STATE and packet.string == "end"):
                _subject, messages = history2messages(game, game.historyGet()[game.history_index:], serial2name = lambda serial: self.serial2name(game, serial))
