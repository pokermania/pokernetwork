...
#        self.destroyDb()
    ...
    def test01_getCurrencySerial_forceExceptionOnRowCount(self):
        ...
        class MockCursor(MockCursorBase):
            def statementActions(cursorSelf, sql, statement):
                if sql[:len(statement)] == "SELECT":
                    cursorSelf.rowCount = 0
                elif sql[:len(statement)] == "INSERT":
                    cursorSelf.rowCount = 0
            def __init__(cursorSelf):
                MockCursorBase.__init__(cursorSelf, self, ["SELECT","INSERT"])
        self.cashier.setDb(MockDatabase(MockCursor))
    ...
    def test02_getCurrencySerial_forceExceptionOnExecute(self):
        ...
        class MockCursor(MockCursorBase):
            def statementActions(cursorSelf, sql, statement):
                if sql[:len(statement)] == "SELECT":
                    cursorSelf.rowcount = 0
                elif sql[:len(statement)] == "INSERT INTO":
                    raise Exception(ER.DUP_ENTRY)
            def __init__(cursorSelf):
                MockCursorBase.__init__(cursorSelf, self, ["SELECT","INSERT INTO"])
        ...
        self.assertEquals(get_messages(), ["SELECT serial FROM currencies WHERE url = 'http://example.org'"])
    # --------------------------------------------------------
    ...
    def test04_cashOutUpdateSafe_twoNullPackets(self):
        ...
        class MockCursor(MockCursorBase):
            def statementActions(cursorSelf, sql, statement):
                cursorSelf.rowcount = 0
                if sql[:len(statement)] in ("DELETE","UPDATE"):
    ...
    def test05_cashOutUpdateSafe_secondPacketGood(self):
        ...
        class MockCursor(MockCursorBase):
            def statementActions(cursorSelf, sql, statement):
                ...
                if sql[:len(statement)] in ("DELETE","UPDATE"):
                    cursorSelf.rowcount = 1
                elif sql[:len(statement)] == ("SELECT") and cursorSelf.counts["SELECT"] == 2:
                    cursorSelf.rowcount = 1
    ...
    def test10_cashOutBreakNote_DeferredFromCommit(self):
        ...
        class MockCursor(MockCursorBase):
            def statementActions(cursorSelf, sql, statement):
                cursorSelf.rowcount = 0
                if sql[:len(statement)] == "SELECT transaction_id":
                    cursorSelf.rowcount = 1
                    cursorSelf.row = (777,)
                elif sql[:len(statement)] == "SELECT serial FROM currencies":
                    cursorSelf.rowcount = 1
                    cursorSelf.row = (6,)
                elif sql[:len(statement)] == "SELECT counter.user_serial":
                    cursorSelf.rowcount = 1
    ...
    def test11_cashOutBreakNote_multirowForSerial(self):
        ...
        class MockCursor(MockCursorBase):
            ...
            def __init__(cursorSelf):
                MockCursorBase.__init__(cursorSelf, self, [
                    "SELECT transaction_id", "SELECT name"
                ])
...
class PokerCashierLockUnlockTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        class  MockLock:
            ...
            def acquire(lockSelf, name, value):
                ...
                if name in lockSelf.acquireCounts:
                    lockSelf.acquireCounts[name] += 1
                else:
                    lockSelf.acquireCounts[name] = 1
    ...
    def test01_unlockNonExistent(self):
        ...
        self.assertEquals(self.cashier.locks, {})
        self.assertEquals(get_messages(), ['cashInUnlock: unexpected missing cash_5 in locks (ignored)'])
    # --------------------------------------------------------
