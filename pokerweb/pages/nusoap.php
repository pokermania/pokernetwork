	...
	*/
    function parseResponse($headers, $data) {
		...
		if (!strstr($headers['content-type'], 'text/xml')) {
			$this->setError('Response not of type text/xml<p>' . $data);
			return false;
		}
    ...
    */
	...
	function UpdateCookies($cookies) {
		...
		foreach ($cookies as $newCookie) {
                  $this->debug("UpdateCookies " . $this->varDump($newCookie));
			...
			for ($i = 0; $i < count($this->cookies); $i++) {
				$cookie = $this->cookies[$i];
                                $this->debug("UpdateCookies compare to " . $this->varDump($cookie));
				if (!is_array($cookie)) {
                                        $this->debug("1");
					continue;
				}
